...
for tp in range(1, interesting_time_points+1):
    print "Generating figure", tp
    fig = plt.figure(0, (6, 6))
    plt.plot(d[:tp], p[:tp, 2], 'b', lw=3, label='Upper pressure')
    plt.plot(d[:tp], p[:tp, 1], 'r', lw=3, label='Lower pressure')
    plt.plot(d[:tp], p[:tp, 0], 'g', lw=3, label='Radial pressure')

    plt.plot(d[tp-1], p[tp-1, 2], 'bp', lw=3, ms=12)
    plt.plot(d[tp-1], p[tp-1, 1], 'rp', lw=3, ms=12)
    plt.plot(d[tp-1], p[tp-1, 0], 'gp', lw=3, ms=12)
    ...
    # plt.tight_layout()
    idx_str = '0'*(len(str(total_time_points))-len(str(filling_tp + tp))) + str(filling_tp + tp)
    plt.savefig('storakers_seminar/animation/graph_figures/graph' + idx_str + '.png')
    fig.clf()
