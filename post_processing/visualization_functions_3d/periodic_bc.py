...
class PeriodicBC:
    ...
    def draw(self, data):
        ...
        for i in range(3):
            if d[i] == 0:
                data[2*i] = self.side_width[0] if self.side_width is not None else np.min(data[[0, 2, 4] != 2*i])
                data[2*i+1] = self.side_width[0] if self.side_width is not None else np.max(data[[1, 3, 5] != 2*i+1])
