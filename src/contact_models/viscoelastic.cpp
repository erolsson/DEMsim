...
DEM::Viscoelastic::Viscoelastic(DEM::Viscoelastic::ParticleType *particle1,DEM::Viscoelastic::ParticleType* particle2,
    ...
    R0_ = 1. / (1. / particle1->get_radius() + 1. / particle1->get_radius());
    double E1 = mat1->E;
    double E2 = mat2->E;
    double v1 = mat1->nu;
    double v2 = mat2->nu;
...
{
    update_normal_force(h-h_);
    update_tangential_force(dt, normal);
}

double DEM::Viscoelastic::update_normal_force(double dh)
{
    if (dh+h_ > -bt_)
    {
        ...
        for (unsigned i=0 ; i != M; ++i)
        {
            ...
            auto h_32diff= pow(h_ + dh + bt_, 3./ 2)-hn32;
