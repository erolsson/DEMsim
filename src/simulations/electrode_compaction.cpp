...
void DEM::electrode_compaction(const std::string& settings_file_name) {
    ...
    //simulator.run(run_for_time);
    EngineType::SurfaceNormalForceLess calendering_force(top_surface, 7.24e7*box_side*box_side);
    simulator.set_rotation(false);
    mat-> adhesive = true;
    ...
    EngineType::RunForTime run_for_time_compact_10(simulator,0.007925s);
    ...
    EngineType::RunForTime run_for_time_relax_10(simulator,200s);
